...
func (c *Controller) Update(ctx context.Context, signer Signer, fn func(UnsignedOp) (UnsignedOp, error)) error {
...
func (c *Controller) Recover(ctx context.Context, signer Signer, forkCID string, fn func(UnsignedOp) (UnsignedOp, error)) error {
...
func (c *Controller) fetchHead(ctx context.Context) (string, UnsignedOp, error) {
	entries, err := c.registry.fetchAuditLog(ctx, c.didStr)
	if err != nil {
		return "", UnsignedOp{}, fmt.Errorf("fetching audit log: %w", err)
	}
	var headCID string
	var headOp *UnsignedOp
	for i := range entries {
	...
	if headCID == "" {
		return "", UnsignedOp{}, fmt.Errorf("no valid operations found for %s", c.didStr)
	}
	if headOp == nil {
		return "", UnsignedOp{}, fmt.Errorf("DID %s is tombstoned", c.didStr)
	}
	return headCID, *headOp, nil
}

// fetchOpAtCID finds and returns the document state at a specific CID in the audit log.
func (c *Controller) fetchOpAtCID(ctx context.Context, cid string) (UnsignedOp, error) {
	entries, err := c.registry.fetchAuditLog(ctx, c.didStr)
	if err != nil {
		return UnsignedOp{}, fmt.Errorf("fetching audit log: %w", err)
	}
	for _, e := range entries {
		if e.CID == cid {
			if e.Op == nil {
				return UnsignedOp{}, nil
			}
			return *e.Op, nil
		}
	}
	return UnsignedOp{}, fmt.Errorf("operation with CID %s not found for %s", cid, c.didStr)
}
