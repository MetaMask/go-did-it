...
// parseCIDToLink decodes a CIDv1 base32 multibase string and validates that it
// uses the dag-cbor codec, returning a cid.Cid ready for DAG-CBOR link encoding.
func parseCIDToLink(cidStr string) (gocid.Cid, error) {
	c, err := gocid.Decode(cidStr)
	if err != nil {
		return gocid.Undef, fmt.Errorf("invalid CID %q: %w", cidStr, err)
	}
	if c.Version() != 1 {
		return gocid.Undef, fmt.Errorf("CID %q: expected CIDv1, got v%d", cidStr, c.Version())
	}
	if c.Type() != uint64(mc.DagCbor) {
		return gocid.Undef, fmt.Errorf("CID %q: expected dag-cbor codec (0x%x), got 0x%x",
